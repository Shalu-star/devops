...
import org.training.core.form.FeedbackForm;
import org.training.facades.feedback.ProductFeedbackFacade;
...
public class ProductPageController extends AbstractPageController
{
	...
	@Resource
	private ProductFeedbackFacade productFeedbackFacade;
	...
	@RequestMapping(value = PRODUCT_CODE_PATH_VARIABLE_PATTERN + "/saveButton", method =
	{ RequestMethod.GET, RequestMethod.POST }) //NOSONAR
	public String postSaveCommentInReview(@ModelAttribute("feedbackForm")
	final FeedbackForm feedbackForm, final RedirectAttributes redirectAttrs, @PathVariable("productCode")
	final String encodedProductCode, final Model model) throws CMSItemNotFoundException
	{
		...
		final String productCode = decodeWithScheme(encodedProductCode, UTF_8);
		//final ProductData productData = productFacade.getProductForCodeAndOptions(productCode, null);
		// productData.setProductFeedbacks(feedbackForm.getProductFeedbacks());//productmodel set
		final ProductData productData = productFeedbackFacade.postProductFeedback(productCode, feedbackForm.getProductFeedbacks());
		model.addAttribute("voucherInputPlaceholder", new FeedbackForm());
		//GlobalMessages.addFlashMessage(redirectAttrs, GlobalMessages.CONF_MESSAGES_HOLDER, "feedback.confirmation.thank.you.title");
		System.out.println("******************************" + "Inside SAVE METHOD " + productData.getProductFeedbacks());
